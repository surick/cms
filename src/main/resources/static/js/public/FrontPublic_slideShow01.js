var FrontPublic_slideShow01 = function() {
     ...
     */
    this.ce_slide_tag_thumbnail_init = function() { //缩略图页签初始化
        ...
        //对比缩略图总长度是否超出幻灯片总长度
        if (parseInt(this.ce_slide_frame.width()) < this.ce_slide_thumbnail_max_width) { //如果超出则展示左右翻页按钮
            //计算展示区域宽度 
    ...
    this.ce_slide_tag_changeStyle = function() {
        //页签轮播变换样式
        var nowObj = null;
        if (this.ce_slide_tag_text.length) { //如果页签数据存在
        ...
        if (this.ce_slide_thumbnails.length) { //如果缩略图数据存在
     ...
     *		  5 向上滚动效果
     */
    this.ce_slide_effect_init = function(style) { //效果类型选择

        switch (style) {
            case 1: //淡出淡入效果
                if (!this.ce_slide_pic_temple) { //如果存储器为空，则表明幻灯为第一次显示
